<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="forum.aspx.cs" Inherits="iSports.asp.forum" %>
...
<html>
    ...
    <div  id="news_right">
    <div  class="reg_right_title" style="position:relative"><b>球赛论坛</b><font>Forum</font>
        <a href="new_theme.aspx" >
        <input onClick="new_theme.aspx" style=" right: 0; top: 11px;" type="button" name="new_topic" id="new_topic" value="新建话题" class="top_btn"/></a>
         ...
         <div class="newslist" style="font-size:1.5rem;">
              ...
              %>
                  <dl class="news_nopic">
                      <dd class="title"><a href="<% Response.Write("bbs.aspx?a="+bbs_item[i,0]);%>"><% Response.Write(bbs_item[i, 1]); %></a></dd>
                      <dd class="content_detail"><% Response.Write(bbs_item[i,2]);%><a href="<% Response.Write("bbs.aspx" + bbs_item[i, 0]); %>"></a></dd>
                  </dl>
              <%
                  }
              %>
            </div>
              <div class="pager">当前是：<% Response.Write(page);%>/<% Response.Write(page_num);%>页 共<%Response.Write(bbs_count);%>条信息
              <%
                  if (page == 1)
                      Response.Write("<span class=''>首页</span> <span class=''>上一页</span>");
                  else {
                %>
              <a href="forum.aspx">首页</a> <a href="?classid=6593&amp;page=<% Response.Write(page - 1);%>">上一页</a>
              <%
            }
            %>
              <% if(page<page_num){%>
